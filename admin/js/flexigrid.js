							 	...
							 	function ()
									{
										...
										if(idx == 8) {
											if(row.cell[9] > 0) {
												td.innerHTML = "<input type='text' size='3' value='" + row.cell[idx] + "' onchange=\"test('Save', "+ row.cell[0] +",this.value);\"><div style='display: none'>" + row.cell[idx] + "</div>";
											}
											else {
		...
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>Quick Search <input type='text' size='30' name='q' id='q' class='qsbox' /> <select name='qtype' id='qtype' onchange='changeSmask(this.value);'>"+sopt+"</select> <input type='button' value='Clear' /></div>");
